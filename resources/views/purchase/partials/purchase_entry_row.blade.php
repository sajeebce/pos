@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            {{-- IMEI/Serial Number Tracking Section --}}
            @if(session('business.enable_imei_tracking'))
                <br>
                <div class="imei_checkbox_wrapper">
                    <label class="checkbox-inline" style="font-weight: normal;">
                        <input type="checkbox"
                            name="purchases[{{$row_count}}][enable_imei_tracking]"
                            value="1"
                            class="imei_tracking_checkbox"
                            data-row="{{$row_count}}"
                            @if(!empty($product->enable_imei_tracking)) checked @endif>
                        <small class="text-info"><i class="fa fa-barcode"></i> @lang('lang_v1.enable_imei_for_purchase')</small>
                    </label>
                </div>
                <div class="imei_input_wrapper" data-row="{{$row_count}}" style="@if(empty($product->enable_imei_tracking)) display:none; @endif">
                    <textarea name="purchases[{{$row_count}}][imei_serial_numbers]"
                        class="form-control input-sm imei_serial_input"
                        rows="2"
                        placeholder="@lang('lang_v1.enter_imei_numbers_placeholder')"
                        data-row="{{$row_count}}"></textarea>
                    <small class="imei_count_display" data-row="{{$row_count}}">
                        <span class="imei_count text-success">0</span> @lang('lang_v1.imei_serial_number')
                        <span class="imei_qty_mismatch text-danger" style="display:none;">
                            <i class="fa fa-exclamation-triangle"></i> @lang('lang_v1.imei_qty_mismatch')
                        </span>
                    </small>
                </div>
            @endif
