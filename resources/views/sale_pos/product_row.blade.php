...
<br>
	...
	<td>
		...
		{{-- IMEI/Serial Number Multi-Select with Lazy Loading --}}
		@if(!empty($product->enable_imei_tracking) && !empty($available_serials_count) && $available_serials_count > 0)
			<br>
			<div class="imei_selection_div" data-row="{{$row_count}}">
				<span style="white-space: nowrap;">
					<small class="text-info"><i class="fa fa-barcode"></i> @lang('lang_v1.select_imei'):</small>
				</span><br>
				<select name="products[{{$row_count}}][selected_serial_ids][]"
					class="form-control input-sm imei_multiselect_lazy"
					multiple="multiple"
					required
					data-row="{{$row_count}}"
					...
					data-available_count="{{$available_serials_count}}"
					data-placeholder="@lang('lang_v1.search_imei')..."
					style="width: 100%;">
				</select>
				<small class="imei_selection_info text-muted">
					<span class="selected_count">0</span> @lang('lang_v1.selected') |
					<span class="available_count">{{$available_serials_count}}</span> @lang('lang_v1.available')
				</small>
			</div>
		@elseif(!empty($product->enable_imei_tracking))
			<br>
			<small class="text-danger"><i class="fa fa-exclamation-triangle"></i> @lang('lang_v1.serial_not_available')</small>
			<input type="hidden" name="products[{{$row_count}}][selected_serial_ids][]" value="">
		@endif
