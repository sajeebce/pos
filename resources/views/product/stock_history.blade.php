    ...
    @component('components.widget', ['title' => $product->name])
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                @php
                    $default_location = request()->input('location_id', $business_locations->keys()->first());
                @endphp
                {!! Form::select('location_id', $business_locations, $default_location, ['class' => 'form-control select2', 'style' => 'width:100%']); !!}
            </div>
        </div>
