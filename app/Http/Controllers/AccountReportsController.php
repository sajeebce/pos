...
class AccountReportsController extends Controller
{
    ...
    public function balanceSheet()
    {
        ...
        if (request()->ajax()) {
            ...
            // Calculate sell return payment due (amount owed to customers)
            $sell_return_payment_due = $this->getSellReturnPaymentDue($business_id, $end_date, $location_id);

            $output = [
                'supplier_due' => $purchase_details['purchase_due'],
                'customer_payment_due' => $sell_return_payment_due,
    ...
    /**
     * Calculate sell return payment due (amount owed to customers after product returns)
     *
     ...
     * @return float
     */
    private function getSellReturnPaymentDue($business_id, $end_date, $location_id = null)
    {
        $query = \App\Transaction::where('transactions.business_id', $business_id)
            ...
            ->select(
                DB::raw('SUM(transactions.final_total) as total_sell_return'),
        ...
        $total_sell_return = $result->total_sell_return ?? 0;
        $total_paid = $result->total_paid ?? 0;

        return $total_sell_return - $total_paid;
    }
