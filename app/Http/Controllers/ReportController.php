...
class ReportController extends Controller
{
    ...
    public function getDueReports(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);

        if ($request->ajax()) {
            $query = TransactionPayment::with(['transaction.contact:id,name,contact_id'])
            ...
            foreach ($payments as $payment) {
                ...
                $data[] = [
                    'customer_name' => $contact?->name ?? 'N/A',
                    'invoice_no' => $transaction->invoice_no ?? '',
                    'sale_date' => $transaction && $transaction->transaction_date
                        ? Carbon::parse($transaction->transaction_date)->format('d-M-Y')
                        : '',
                    'payment_date' => $payment && $payment->paid_on
                        ? Carbon::parse($payment->paid_on)->format('d-M-Y')
                        : '',
                    'paid_amount' => number_format($payment->amount, 2),
        ...
        return view('report.due_report', compact('business_locations'));
    }

    public function getStockInitReports(Request $request)
    {
        ...
        // 1️⃣ Build ascending date list (for proper calculations)
        $dates = collect();
        ...
        $previous_total_products = 0;

        // 2️⃣ Process in chronological order
        foreach ($dates as $date) {
            // Total products up to this date
            $total_products = Product::where('business_id', $business_id)
                ->when($location_id, fn($q) => $q->where('location_id', $location_id))
                ->whereDate('created_at', '<=', $date)
                ->count();

            // New products added this date
            $new_products = Product::where('business_id', $business_id)
                ->when($location_id, fn($q) => $q->where('location_id', $location_id))
                ->whereDate('created_at', $date)
                ->count();
            ...
            // Closing stock = stock value up to this date
            $closing_stock = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                ->leftJoin('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
                ->where('p.business_id', $business_id)
                ->when($location_id, fn($q) => $q->where('vld.location_id', $location_id))
                ->whereDate('p.created_at', '<=', $date)
                ->sum(DB::raw('vld.qty_available * variations.default_purchase_price'));
            ...
            // Update for next day
            $previous_closing_stock = $closing_stock;
            $previous_total_products = $total_products;
        }

        // 3️⃣ Sort dates DESC — but keep computed values correct
        ...
        return view('report.stock_report_init', compact(
            'report',
            'start_date',
            'end_date',
            'business_locations'
    ...
    public function getDueCheckingReports(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        ...
        // Default to today if no date_range
        ...
        // Query transactions (due + partial)
        ...
        foreach ($transactions as $t) {
            $total_invoice += $t->final_total;
            $paid = $t->payment_lines->sum('amount');
            $total_paid += $paid;
            $total_due += max($t->final_total - $paid, 0);
        }

        return view('report.due_checking_report', compact(
            'business_locations',
            'transactions',
            'total_invoice',
            'total_paid',
