...
class AdminlteCustomPresenter extends Presenter
{
    ...
    {
        return '<a href="' . $item->getUrl() . '" title="" class="tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-tracking-tight text-white-white tw-transition-all tw-duration-200 tw-rounded-lg tw-whitespace-nowrap' . $this->getActiveState($item) . '" ' . $item->getAttributes() . '>' .
            $this->formatIcon($item->icon) . ' <span class="text-white-white">' . $item->title . '</span>' .
            '</a>' . PHP_EOL;
    }
    ...
    {
        $dropdownToggle = '<a href="#" title="" class="drop_down tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-tracking-tight text-white-white tw-transition-all tw-duration-200 tw-rounded-lg tw-whitespace-nowrap focus:tw-text-gray-900 focus:tw-bg-gray-100' . $this->getActiveStateOnChild($item) . '" ' . $item->getAttributes() . '>' .
            $this->formatIcon($item->icon) . ' <span class="text-white-white">' . $item->title . '</span>' .
            '<svg aria-hidden="true" class="svg tw-ml-auto tw-text-white tw-size-4 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">' . $this->getArray($item) .
            '</svg>' .
            '</a>';
    ...
    public function getChildMenuItems($item)
    {
        ...
        if (count($item->getChilds()) > 0) {
            ...
            foreach ($item->getChilds() as $child) {
                ...
                $children .= '<a href="' . $child->getUrl() . '" title="" class="tw-flex tw-text-sm tw-font-medium tw-tracking-tight text-white-white tw-truncate tw-transition-all tw-duration-200 hover:tw-text-gray-900 tw-whitespace-nowrap ' . $isActive . '"' . $isActive . ' "' . $child->getAttributes() . '"' . $child->hasActiveOnChild() . '>' .
    ...
    protected function formatIcon($icon)
    {
        ...
        // Check if the icon string contains "images/", indicating it's an image icon
        elseif (strpos($icon, 'images/') !== false) {
            return '<img src="' . asset($icon) . '" alt="Icon" class="tw-size-5 tw-shrink-0">';
        } else {
