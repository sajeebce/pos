...
class ProductUtil extends Util
{
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        // Save IMEI/Serial numbers for products with IMEI tracking enabled
        foreach ($input_data as $key => $data) {
            if (! empty($data['enable_imei_tracking']) && ! empty($data['imei_serial_numbers'])) {
                $purchase_line = $updated_purchase_lines[$key] ?? null;
                if ($purchase_line && $purchase_line->id) {
    ...
    /**
     * Save serial numbers (IMEI) for a purchase line
     *
     * @param  int  $business_id
     * @param  int  $product_id
     * @param  int  $variation_id
     ...
     * @return void
     */
    public function saveSerialNumbers($business_id, $product_id, $variation_id, $location_id, $purchase_line_id, $serial_numbers_string, $purchase_date)
    {
        // Parse serial numbers (one per line or comma separated)
        $serial_numbers = $this->parseSerialNumbers($serial_numbers_string);
        ...
        foreach ($serial_numbers as $serial_number) {
            if (! empty($serial_number)) {
                // Check if serial number already exists for this business
                $exists = \App\ProductSerial::where('business_id', $business_id)
                    ->where('serial_number', $serial_number)
                    ->exists();

                if (! $exists) {
                    \App\ProductSerial::create([
                        'business_id' => $business_id,
                        'product_id' => $product_id,
                        'variation_id' => $variation_id,
                        'location_id' => $location_id,
                    ...
                    $duplicate_serials[] = $serial_number;
                }
            }
        }
        ...
        return [
            ...
            'total_count' => count($serial_numbers),
     ...
     * @return array
     */
    public function parseSerialNumbers($serial_numbers_string)
    {
        if (empty($serial_numbers_string)) {
            return [];
        }
        ...
        return array_values($serial_numbers);
    }
     ...
     * @return array
     */
    public function validateSerialNumbers($business_id, $serial_numbers_string)
    {
        $serial_numbers = $this->parseSerialNumbers($serial_numbers_string);
        ...
        foreach ($serial_numbers as $serial_number) {
            ...
            if ($exists) {
                $duplicates_in_system[] = $serial_number;
            }
        }

        if (! empty($duplicates_in_input)) {
            $errors[] = __('lang_v1.duplicate_imei_found', ['imei' => implode(', ', array_unique($duplicates_in_input))]);
        }

        if (! empty($duplicates_in_system)) {
            $errors[] = __('lang_v1.imei_already_in_system', ['imei' => implode(', ', $duplicates_in_system)]);
        }

        return [
            'is_valid' => empty($errors),
            'errors' => $errors,
            'duplicates_in_input' => array_unique($duplicates_in_input),
            'duplicates_in_system' => $duplicates_in_system,
            'parsed_count' => count($serial_numbers),
        ];
    }
     ...
     * @param  string  $sold_date
     * @return void
     */
    public function markSerialAsSold($serial_id, $sell_line_id, $sold_date)
    {
        $serial = \App\ProductSerial::find($serial_id);
        if ($serial) {
            $serial->sell_line_id = $sell_line_id;
            $serial->status = 'sold';
            $serial->sold_date = $sold_date;
            $serial->save();
        }
    }

    /**
     * Get available serial numbers for a product variation at a location
     *
     * @param  int  $business_id
     * @param  int  $product_id
     * @param  int  $variation_id
     ...
     */
    public function getAvailableSerials($business_id, $product_id, $variation_id, $location_id)
    {
        return \App\ProductSerial::where('business_id', $business_id)
            ->where('product_id', $product_id)
            ->where('variation_id', $variation_id)
            ->where('location_id', $location_id)
