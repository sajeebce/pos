...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                ...
                $total_line_taxes += ($line['tax_unformatted'] * (int) $line['quantity']);
            }
        }
    ...
    /**
     * Handle IMEI/Serial Number return - mark serials as available again
     *
     * @param  int  $sell_line_id
    ...
    public function handleSerialNumberReturn($sell_line_id, $quantity_returned, $quantity_before = 0, $return_serial_ids = [])
    {
        ...
        if ($quantity_returned > $quantity_before) {
            ...
            // Get the sold serials for this sell line
            $serials = \App\ProductSerial::where('sell_line_id', $sell_line_id)
                ->where('status', 'sold')
                ->limit($additional_returns)
                ->get();
        ...
        elseif ($quantity_returned < $quantity_before) {
            // This case is handled by the quantity update - serials remain in their current state
            // If needed, implement logic to re-mark serials as sold
        }
    }

    /**
     * Mark serials as available (can be resold)
     * Used when you want returned items to be available for sale again
     *
     * @param  int  $sell_line_id
     * @return int  Number of serials made available
     */
    public function makeReturnedSerialsAvailable($sell_line_id)
    {
        return \App\ProductSerial::where('sell_line_id', $sell_line_id)
