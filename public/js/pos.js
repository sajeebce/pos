        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    // Initialize IMEI lazy loading Select2 for the new row
                    if (typeof initImeiMultiSelectLazy === 'function') {
                        this_row.find('.imei_multiselect_lazy').each(function() {
                            initImeiMultiSelectLazy($(this));
                        });
                    }
                } else {
                    toastr.error(result.msg);
...
// ==========================================
// IMEI/Serial Number Multi-Select Functions
// ==========================================

/**
 * Initialize IMEI multi-select dropdown with Select2
 */
function initImeiMultiSelect(element) {
    if (!element || element.length === 0) return;

    element.select2({
        placeholder: element.data('placeholder') || 'Search IMEI...',
        allowClear: true,
        closeOnSelect: false,
        width: '100%',
        dropdownParent: element.closest('.imei_selection_div'),
        templateResult: formatImeiOption,
        templateSelection: formatImeiSelection
    });

    // Update selected count on change
    element.on('change', function() {
        updateImeiSelectionInfo($(this));
        syncQuantityWithImeiSelection($(this));
    });
}

/**
 * Format IMEI option in dropdown
 */
function formatImeiOption(state) {
    if (!state.id) {
        return state.text;
    }
    return $('<span><i class="fa fa-barcode text-info"></i> ' + state.text + '</span>');
}

/**
 * Format selected IMEI
 */
function formatImeiSelection(state) {
    if (!state.id) {
        return state.text;
    }
    return state.text;
...
/**
 * Update IMEI selection info (selected count / available count)
 */
function updateImeiSelectionInfo(selectElement) {
    var row = selectElement.closest('tr');
    var selectedCount = selectElement.val() ? selectElement.val().length : 0;
    var availableCount = selectElement.find('option').length;

    row.find('.selected_count').text(selectedCount);
    row.find('.available_count').text(availableCount);
}

/**
 * Sync quantity with selected IMEI count
 */
function syncQuantityWithImeiSelection(selectElement) {
    var row = selectElement.closest('tr');
    var selectedCount = selectElement.val() ? selectElement.val().length : 0;

    if (selectedCount > 0) {
        var quantityInput = row.find('input.pos_quantity');
        var currentQty = parseFloat(quantityInput.val()) || 0;

        if (currentQty !== selectedCount) {
            quantityInput.val(selectedCount);
...
/**
 * Initialize all IMEI multi-selects on page
 */
function initAllImeiMultiSelects() {
    $('.imei_multiselect').each(function() {
        if (!$(this).hasClass('select2-hidden-accessible')) {
            initImeiMultiSelect($(this));
        }
    });
}

// Initialize IMEI multi-selects when product row is added
$(document).on('DOMNodeInserted', '#pos_table tbody', function(e) {
    var newRow = $(e.target);
    if (newRow.is('tr')) {
        setTimeout(function() {
            newRow.find('.imei_multiselect').each(function() {
                if (!$(this).hasClass('select2-hidden-accessible')) {
                    initImeiMultiSelect($(this));
                }
            });
...
$(document).ready(function() {
    initAllImeiMultiSelects();
});

// Validate IMEI selection before form submission
$(document).on('submit', 'form#add_pos_sell_form, form#edit_pos_sell_form', function(e) {
    var hasImeiError = false;
    var errorMessages = [];

    // Check each product row with IMEI enabled
    $('#pos_table tbody tr').each(function() {
        var row = $(this);
        var imeiSelect = row.find('.imei_multiselect');

        if (imeiSelect.length > 0) {
            var selectedCount = imeiSelect.val() ? imeiSelect.val().length : 0;
            var quantity = parseFloat(row.find('input.pos_quantity').val()) || 0;
            var productName = row.find('.product_name').text() || 'Product';

            if (quantity > 0 && selectedCount !== quantity) {
                hasImeiError = true;
                errorMessages.push(productName.trim() + ': Please select ' + quantity + ' IMEI(s). Currently selected: ' + selectedCount);
            }
        }
    });
...
// When quantity changes in POS, check IMEI selection
$(document).on('change', '.pos_quantity', function() {
    var row = $(this).closest('tr');
    var imeiSelect = row.find('.imei_multiselect, .imei_multiselect_lazy');
    ...
    if (imeiSelect.length > 0) {
        ...
        if (selectedCount > 0 && selectedCount !== quantity) {
            toastr.warning('IMEI selection count (' + selectedCount + ') does not match quantity (' + quantity + ')');
        }
    }
});

// ==========================================
// IMEI Multi-Select with Lazy Loading (AJAX)
// ==========================================

/**
 * Initialize IMEI multi-select dropdown with Select2 and AJAX lazy loading
 * @param {jQuery} element - The select element to initialize
 */
function initImeiMultiSelectLazy(element) {
    if (!element || element.length === 0) return;
    if (element.hasClass('select2-hidden-accessible')) return; // Already initialized

    var variationId = element.data('variation_id');
    var locationId = element.data('location_id');
    var placeholder = element.data('placeholder') || 'Search IMEI...';
    var availableCount = element.data('available_count') || 0;
    ...
    element.select2({
        ...
        dropdownParent: element.closest('.imei_selection_div'),
        ajax: {
            url: '/sells/pos/get-available-serials',
            dataType: 'json',
            delay: 250,
            data: function(params) {
                return {
        ...
        templateResult: formatImeiOptionLazy,
        templateSelection: formatImeiSelectionLazy
    });

    // Update selected count and sync quantity on change
    element.on('change', function() {
        updateImeiSelectionInfoLazy($(this));
        syncQuantityWithImeiSelectionLazy($(this));
    });
}

/**
 * Format IMEI option in dropdown (lazy loading version)
 */
function formatImeiOptionLazy(state) {
    if (state.loading) {
        return state.text;
    }
    if (!state.id) {
    ...
    var $container = $('<span><i class="fa fa-barcode text-info"></i> <code>' + state.text + '</code></span>');
    return $container;
}

/**
 * Format selected IMEI (lazy loading version)
 */
function formatImeiSelectionLazy(state) {
    if (!state.id) {
        return state.text;
    }
    return state.text;
...
/**
 * Update IMEI selection info (selected count / available count) - lazy loading version
 */
function updateImeiSelectionInfoLazy(selectElement) {
    var row = selectElement.closest('tr');
    var selectedCount = selectElement.val() ? selectElement.val().length : 0;
    var availableCount = selectElement.data('available_count') || 0;
...
/**
 * Sync quantity with selected IMEI count - lazy loading version
 */
function syncQuantityWithImeiSelectionLazy(selectElement) {
    var row = selectElement.closest('tr');
    var selectedCount = selectElement.val() ? selectElement.val().length : 0;
...
/**
 * Initialize all IMEI multi-selects with lazy loading on page
 */
function initAllImeiMultiSelectsLazy() {
    $('.imei_multiselect_lazy').each(function() {
        initImeiMultiSelectLazy($(this));
    });
}

// Initialize lazy loading IMEI multi-selects when product row is added
$(document).on('DOMNodeInserted', '#pos_table tbody', function(e) {
    var newRow = $(e.target);
    if (newRow.is('tr')) {
        setTimeout(function() {
            newRow.find('.imei_multiselect_lazy').each(function() {
                initImeiMultiSelectLazy($(this));
            });
        }, 100);
    }
...
// Also initialize lazy loading IMEI on document ready
$(document).ready(function() {
    initAllImeiMultiSelectsLazy();
});

// Validate IMEI selection (lazy loading) before form submission
$(document).on('submit', 'form#add_pos_sell_form, form#edit_pos_sell_form', function(e) {
    var hasImeiError = false;
    var errorMessages = [];

    // Check each product row with IMEI lazy loading enabled
    $('#pos_table tbody tr').each(function() {
        var row = $(this);
        var imeiSelect = row.find('.imei_multiselect_lazy');

        if (imeiSelect.length > 0) {
            var selectedCount = imeiSelect.val() ? imeiSelect.val().length : 0;
            var quantity = parseFloat(row.find('input.pos_quantity').val()) || 0;
            var productName = row.find('td:first').text().trim().split('\n')[0] || 'Product';
