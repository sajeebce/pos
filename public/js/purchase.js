...
// ==========================================
// IMEI/Serial Number Tracking Functions
// ==========================================
...
/**
 * Update IMEI count display and check quantity match
 */
function updateImeiCount(textarea) {
    var row = textarea.closest('tr');
    var rowNum = textarea.data('row');
    var imeis = parseImeiInput(textarea.val());
    var imeiCount = imeis.length;

    // Update count display
    row.find('.imei_count').text(imeiCount);

    // Get current quantity
    var quantity = __read_number(row.find('.purchase_quantity'), true);

    // Check for quantity mismatch
    var mismatchWarning = row.find('.imei_qty_mismatch');
    if (imeiCount > 0 && imeiCount !== quantity) {
...
/**
 * Check for duplicate IMEIs within the input
 */
function checkDuplicateImeis(textarea) {
...
// IMEI Checkbox Toggle - Show/Hide IMEI input
$(document).on('change', '.imei_tracking_checkbox', function() {
    var row = $(this).closest('tr');
    var rowNum = $(this).data('row');
    var imeiWrapper = row.find('.imei_input_wrapper[data-row="' + rowNum + '"]');

    if ($(this).is(':checked')) {
        imeiWrapper.slideDown(200);
    } else {
        imeiWrapper.slideUp(200);
        // Clear IMEI input when unchecked
        imeiWrapper.find('.imei_serial_input').val('');
        row.find('.imei_count').text('0');
        row.find('.imei_qty_mismatch').hide();
    }
});
...
$(document).on('input keyup', '.imei_serial_input', function() {
    var textarea = $(this);
    var row = textarea.closest('tr');
    var imeiCount = updateImeiCount(textarea);

    // Check if IMEI tracking is enabled for this row
    var checkbox = row.find('.imei_tracking_checkbox');
    if (checkbox.is(':checked') && imeiCount > 0) {
        // Auto-sync quantity with IMEI count
        var quantityInput = row.find('.purchase_quantity');
        var currentQty = __read_number(quantityInput, true);

        if (currentQty !== imeiCount) {
            __write_number(quantityInput, imeiCount);
            quantityInput.trigger('change');
        }

        // Hide mismatch warning since we're syncing
        row.find('.imei_qty_mismatch').hide();
    }
});

// When quantity changes, check IMEI count match
$(document).on('change', '.purchase_quantity', function() {
    var row = $(this).closest('tr');
    var checkbox = row.find('.imei_tracking_checkbox');

    if (checkbox.length && checkbox.is(':checked')) {
        var textarea = row.find('.imei_serial_input');
        if (textarea.length) {
            updateImeiCount(textarea);
        }
    }
});

// Validate IMEIs before form submission
$(document).on('click', 'button#submit_purchase_form', function(e) {
    var hasImeiError = false;
    var errorMessages = [];

    // Check each row with IMEI tracking enabled
    $('#purchase_entry_table tbody tr').each(function() {
        ...
        if (checkbox.length && checkbox.is(':checked')) {
            var textarea = row.find('.imei_serial_input');
            var productName = row.find('td:eq(1)').text().trim().split('\n')[0];
            ...
            if (textarea.length) {
                ...
                if (imeis.length !== quantity) {
                    hasImeiError = true;
                    errorMessages.push(productName + ': IMEI count (' + imeis.length + ') must match quantity (' + quantity + ')');
                }

                // Check for duplicates within input
                var dupCheck = checkDuplicateImeis(textarea);
                if (dupCheck.hasDuplicates) {
                    hasImeiError = true;
                    errorMessages.push(productName + ': Duplicate IMEIs found: ' + dupCheck.duplicates.join(', '));
                }
            }
        }
    });
